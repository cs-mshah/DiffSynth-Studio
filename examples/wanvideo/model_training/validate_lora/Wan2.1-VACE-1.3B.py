...
seed = 42
np.random.seed(42)
output_dir = "data/example1/outputs/baseline_ref_img/"
input_dir = "data/example1/preprocess/"
df = pd.read_csv("data/example1/preprocess/metadata.csv")
os.makedirs(output_dir, exist_ok=True)
num_inference = 1
# lora_path = "models/train/Wan2.1-VACE-1.3B_lora/example1/no_prompt/epoch-4.safetensors"
...
print(f'starting inference for {len(df)} videos...')
for index, row in tqdm(df.iterrows()):
    ...
    video_path = os.path.join(input_dir, row["video"])
    reference_image = VideoData(video_path, height=480, width=832)[0]
    prompt = row["prompt"]
